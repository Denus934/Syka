...
{ 
    ...
    case IOCTL_FIND_СHAR: {
        /* Эта ioctl является и вводом (ioctl_param), и выводом 
         * (возвращаемым значением этой функции). 
         */ 
        int count = 0;
        char tofind = ioctl_param;
        for (int i = 0 ; message[i] && i < BUF_LEN; i++) 
        {
            if (message[i] == tofind)
                count++;
        }
    ...
    case IOCTL_FIND_STR: {
        ...
        char temp[100];
        char __user *tmp = (char __user *)ioctl_param; 
        copy_from_user(temp, tmp, 100); 
        char* res = strstr(message, temp);
        ret = res-message; //позиция первого символа
        break; 
    } 
    }
